    ...
    if not (trial in exclude_trial):
        ...
        for row in data:
            ...
            if not row[0] == '0':
                ...
                charge_calc = (float(row[1]) - float(row[0])) / float(row[2]) * pulse_width * math.fabs(
        ...
        mydata[trial_name] = {'start_stim': start_stim, 'end_stim': end_stim, 'amp_stim': amp_stim,
                              'charge_stim': charge_stim, 'charge2_stim': charge2_stim, 'start_move': start_move,
                              'start_walk': start_walk, 'end_move': end_move}
...
p_walk = n_walk / float(
    stim_count) #%of walking rat maybe greater than 100% if the number of walk is superior to the number of stimulation
