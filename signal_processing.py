...
class Signal_processing:
	def __init__(self,save,img_ext):
		self.save=save
	...
	def find_spikes(self,signal,a_spike,b_spike,tresh):
		...
		for i in signal:
			...
			if len(list)>(a_spike+b_spike):
				del list[0]
				if list[b_spike]<tresh and (cpt-last_spike)>a_spike:
	...
	def find_spike_template_kohonen(self,spikes_values,col,row,weight_count,max_weight,alpha,neighbor,min_win,dist_treshold,ext_img,save):
		print('\n## kohonen ##')
		
		self.map=nn.Kohonen(col,row,weight_count,max_weight,alpha,neighbor,min_win,ext_img,save)
		iteration_count=0
		i=0
		...
		#self.map.find_cluster_center(spikes_values,60)
		return self.map
